...
//  Adapted from Markdown Preview (MIT License).
//
...
nonisolated enum MarkdownFrontmatter {
    ...
    static func split(_ markdown: String) -> SplitResult {
        ...
        else { return SplitResult(raw: nil, format: nil, body: markdown) }
        ...
        return SplitResult(raw: raw, format: delimiter.format, body: body)
    }
    ...
    private static func parseYaml(_ raw: String) -> [FrontmatterEntry] {
        ...
        for rawLine in raw.split(separator: "\n", omittingEmptySubsequences: false) {
            ...
            if let flowItems = flowSequenceItems(value) {
                entries.append(ParsedEntry(key: key, value: "", items: flowItems))
                continue
            }
            ...
            entries.append(ParsedEntry(key: key, value: unquote(value), items: []))
        }
